...
Instead, networks are mostly displayed as different kinds of matrices. How these relate to one another can be confusing and is something we don't usually have to deal with in standard quant research. There are three types of matrices that can be used to describe a network and 'translating' them into the desired format varies by type. Often your data isn't even a matrix yet. In that case you first have to figure out in which of the following formats you can/should transform it.   
...
ggraph(soc_sub, layout = "fr") +
  geom_edge_link() +
  geom_node_point(aes(color = as.factor(dep_cat))) +
  theme_graph(base_family="sans")

ggraph(soc_sub, layout = "fr") +
  geom_edge_link() +
  geom_node_point(aes(color = as.factor(dep_cat), size = degree)) +
  geom_node_text(aes(filter = degree>=50, label =  name), 
                 size = 3, repel = TRUE) +
  theme_graph(base_family="sans")

ggraph(soc_sub, layout = "fr") +
  geom_edge_link() +
  geom_node_point(aes(color = as.factor(dep_cat), size = betweenness)) +  
  geom_node_text(aes(filter = betweenness>=50000, label =  name), 
                 size = 3, repel = TRUE) +
  theme_graph(base_family="sans")

ggraph(soc_sub, layout = "fr") +
  geom_edge_link() +
  geom_node_point(aes(color = as.factor(dep_cat), size = eigen)) +
  geom_node_text(aes(filter = eigen>=0.1, label =  name), 
                 size = 3, repel = TRUE) +
  theme_graph(base_family="sans")
```
...
```{r visnetwork, echo=TRUE,  out.extra='style="width:auto;"'}
node_df <- soc_sub %N>% 
  mutate(id = c(1:240)) %>% 
  rename(label = name) %>% 
  as.data.frame() 

edge_df <- soc_sub %E>% as.data.frame() 
