...
auth_att %<>%
  ...
  mutate(dep_cat = case_when(dep_cat == 1 ~ "Theory",
...
visNetwork(node_df, edge_df, width = "100%", height = "600px") %>% 
  ...
  visGroups(groupname = "Theory", color = pal[1]) %>% # have to specify groups individually
  visGroups(groupname = "Polit. Economy", color = pal[3]) %>% 
  visGroups(groupname = "Ineq. in Welfare Societies", color = pal[4]) %>% 
  visGroups(groupname = "Life Course", color = pal[5]) %>% 
  visGroups(groupname = "Health", color = pal[6]) %>% 
