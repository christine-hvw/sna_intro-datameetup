...
knitr::include_graphics("connect_simpsons.gif")
```
...
knitr::include_graphics("data_hard.jpg")
```
...
```{r socpub_edgeset}
readRDS("socpub_edgeset.RDS")

mat <- soc_pub.bipartite.edges %*% t(soc_pub.bipartite.edges) # inc. matrix x transponse(inc. matrix) = adj. matrix

soc_adj <- as_tbl_graph(mat, directed = FALSE)

soc_elist <- as_tbl_graph(soc_pub.bipartite.edges, directed = FALSE)

```

```{r socpub_prep, echo=TRUE}
...
colnames(soc_pub.bipartite.edges) <- paste(soc_pub$title, soc_pub$year, sep = "; ")
  
mat <- soc_pub.bipartite.edges %*% t(soc_pub.bipartite.edges)

soc_pub.bipartite.edges <- soc_pub.bipartite.edges * 1 # convert logical to numeric for tbl_graph

saveRDS(soc_pub.bipartite.edges, file = "socpub_edgeset.RDS")  
